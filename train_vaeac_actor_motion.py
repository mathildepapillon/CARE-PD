...
Trains a VAEAC on the ACTOR Transformer backbone.  The reconstruction path
is **identical** to ACTOR CVAE (Petrovich et al., ICCV 2021):
Encoder_TRANSFORMER produces global z (B, D) via reparameterisation, and
Decoder_TRANSFORMER reconstructs from z alone (z-only mode, no frame_tokens).

The only addition is a masked encoder r_ψ trained by forward KL to align with
q_φ.  At inference, r_ψ replaces q_φ.

Architecture
------------
q_φ(z | x, y)       VaeacActorFullEncoder — wraps Encoder_TRANSFORMER.
                     Produces mu, logvar ∈ ℝ^{B×D}.
r_ψ(z | x_S, S, y)  VaeacActorMaskedEncoder — Encoder_TRANSFORMER with masked
                     input.  Produces its own mu, logvar.
p_θ(x̂ | z, y)       Decoder_TRANSFORMER (unchanged from ACTOR).

Objective
---------
L = 1.0 · MSE(p_θ(z_φ), x)           full-sequence reconstruction
    + λ_vel · velocity_loss            full-sequence velocity
    + λ_kl_prior · KL(q_φ ‖ N(0,1))  matches ACTOR's KL term exactly
    + λ_kl  · KL(q_φ ‖ r_ψ)          aligns r_ψ to q_φ (q_φ detached)
    + λ_reg · prior_reg(r_ψ)          Ivanov prior stabiliser

Gradient routing:
    q_φ: reconstruction + velocity + KL(q_φ ‖ N(0,1)).
    r_ψ: KL(q_φ ‖ r_ψ) + prior regularisation only.
...
class ImputationGifCallback(pl.Callback):
    ...
    def on_validation_batch_end(self, trainer, pl_module, outputs, batch,
        ...
        if pl_module.data_mode == "h36m_rot6d":
            b = actor_batch_from_h36m_rot6d(
                x[:n].float().to(device), pad_mask[:n].to(device), device,
            )
        else:
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        n_saved = 0
        nj = pl_module.model.njoints
        try:
            with torch.no_grad():
                for group_name in self.VIZ_GROUPS:
                    joint_ids = H36M_GROUPS.get(group_name)
                    if joint_ids is None or any(j >= nj for j in joint_ids):
                        continue
                    cm_j = torch.ones(nj, dtype=torch.bool, device=device)
                    ...
                    for bi in range(b["x"].shape[0]):
                        ...
                        fk = pl_module.model.rotation2xyz
                        if fk is not None:
                            m1 = mask1[:, :real_len]
                            gt_xyz = fk(x1[:, :, :, :real_len], m1)
                            out_xyz = fk(x_hat[:, :, :, :real_len], m1)
                            gt_btj3 = gt_xyz.permute(0, 3, 1, 2).cpu().numpy()
                            out_btj3 = out_xyz.permute(0, 3, 1, 2).cpu().numpy()
                        else:
...
class VaeacActorMotionModule(pl.LightningModule):
    ...
    The reconstruction path (q_φ → z → decoder) is identical to ACTOR:
    same architecture, same LR, same loss weights.  The only addition is
    the masked encoder r_ψ trained by forward KL.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.model = model
        self.lr = lr
        self.lr_full_enc = lr_full_enc if lr_full_enc is not None else lr
        self.data_mode = data_mode
        self.lambda_kl = lambda_kl
        self.lambda_kl_prior = lambda_kl_prior
        ...
        self.lambda_velxyz = lambda_velxyz
    ...
    def _build_actor_batch(self, x, lab, pad_mask):
        device = x.device
        if self.data_mode == "h36m_rot6d":
            b = actor_batch_from_h36m_rot6d(x, pad_mask.bool(), device)
        else:
    ...
    def _effective_lambda_kl(self) -> float:
        if self._in_ae_warmup:
            return 0.0
        vaeac_epoch = self.current_epoch - self.ae_warmup_epochs
        if self.kl_n_cycles > 0:
            cycle_len = max(1, self.total_epochs // self.kl_n_cycles)
            pos_in_cycle = vaeac_epoch % cycle_len
        ...
        frac = min(1.0, vaeac_epoch / max(1, self.kl_warmup_epochs))
        return frac * self.lambda_kl
    ...
    def _step(self, batch, train: bool):
        ...
        if self._in_ae_warmup:
            b["coalition_mask"] = None
        elif self.mask_axis == "temporal":
            b["coalition_mask"] = sample_temporal_training_mask_mixed(B, T, device)
        elif self.mask_axis == "both":
            if torch.rand(1).item() < 0.5:
                b["coalition_mask"] = sample_spatial_training_mask(B, device, n_joints=nj)
            else:
                b["coalition_mask"] = sample_temporal_training_mask_mixed(B, T, device)
        else:
            b["coalition_mask"] = sample_spatial_training_mask(B, device, n_joints=nj)
        ...
        loss, ld = self.model.compute_loss(
            ...
            lambda_kl_prior=self.lambda_kl_prior,
            lambda_rcxyz=self.lambda_rcxyz,
            lambda_velxyz=self.lambda_velxyz,
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if (
            ...
            x, lab, _vidx, _meta, pad_mask = batch
            n = min(n_div, x.shape[0])
            b = self._build_actor_batch(x[:n], lab[:n], pad_mask[:n])
            self._div_batch = {
                k: v.cpu() if isinstance(v, torch.Tensor) else v
                for k, v in b.items()
            }
    ...
    def on_train_epoch_start(self):
        if self.ae_warmup_epochs > 0 and self.current_epoch == self.ae_warmup_epochs:
            if getattr(self.trainer, "global_rank", 0) == 0:
                print(
                    f"\n{'='*60}\n"
                    f"[ACTOR warm-up complete at epoch {self.current_epoch}] "
                    f"Switching to full VAEAC training "
                    f"(masked encoder + KL).\n"
                    f"{'='*60}\n",
                    flush=True,
                )
    ...
    def on_validation_epoch_end(self):
        ...
        phase_tag = "AE-warmup" if self._in_ae_warmup else "VAEAC"
        parts = [f"phase={phase_tag}"]
        ...
        if self._div_batch is not None:
            ...
            nj = self.model.njoints
            cm_div = torch.ones(1, nj, dtype=torch.bool, device=device)
            cm_div[0, :min(4, nj)] = False
            masked_idx = (~cm_div[0]).nonzero(as_tuple=True)[0]

            # ---- completion diversity (use first sample only) ----
            with torch.no_grad():
                completions = self.model.sample_completions(
                    b["x"][:1], b["y"][:1], b["mask"][:1], b["lengths"][:1],
            ...
            # ---- inference-mode imputation motion (first sample) ----
            with torch.no_grad():
                b1 = {k: v[:1] if isinstance(v, torch.Tensor) else v
                      for k, v in b.items()}
                b_infer = {**b1, "coalition_mask": cm_div}
            ...
            with torch.no_grad():
                n_probe = 8
                # Masked encoder: same sample, different random masks
                mu_masked_list = []
                for _ in range(n_probe):
                    cm_rand = sample_spatial_training_mask(1, device, n_joints=nj)
                    out_m = self.model.masked_encoder({
                        "x": b["x"][:1], "y": b["y"][:1], "mask": b["mask"][:1],
                ...
                # Full encoder: different sequences (cross-sample diversity)
                out_f = self.model.full_encoder({
                    "x": b["x"], "y": b["y"], "mask": b["mask"],
                })
                mu_full_t = out_f["mu_full"]  # (N, D)
                mu_div_phi = float(torch.cdist(mu_full_t, mu_full_t).mean())

            # ---- σ diagnostics (logvar is (B, D) now) ----
            with torch.no_grad():
                fixed_cm = torch.ones(1, nj, dtype=torch.bool, device=device)
                fixed_cm[0, :min(4, nj)] = False
...
def parse_args():
    p = argparse.ArgumentParser(
        description="VaeacActorMotion training — VAEAC on ACTOR backbone (global z + deterministic frame_tokens).",
    ...
    p.add_argument(
        ...
        choices=("carepd", "h36m_rot6d"),
        help="carepd: H36M XYZ (17×3). h36m_rot6d: H36M 6D rotations (32×6).",
    )
    ...
    p.add_argument("--lr_full_enc", type=float, default=None,
                   help="q_φ backbone LR (default: same as --lr, matching ACTOR).")
    ...
    p.add_argument(
        ...
        help="z-only decoder mode (matching original ACTOR paper). "
             "Frame tokens from the encoder are NOT forwarded to the decoder. "
             "The decoder reconstructs from z alone, which prevents the "
             "cross-attention averaging that causes mean-pose collapse.",
    )
    ...
    p.add_argument("--ae_warmup_epochs", type=int, default=0,
                   help="Pure ACTOR warm-up: for the first N epochs the model "
                        "trains without coalition masks (no masked encoder, no KL) "
                        "— identical to vanilla ACTOR CVAE training.  Set 0 to "
                        "start full VAEAC training from epoch 0 (recommended, "
                        "since the training path already replicates ACTOR).")
    p.add_argument("--lambda_kl", type=float, default=0.01,
                   help="KL weight for forward KL(q_φ ‖ r_ψ) on global latent.")
    p.add_argument("--lambda_kl_prior", type=float, default=1e-5,
                   help="KL(q_φ ‖ N(0,1)) weight — matches ACTOR's lambda_kl. "
                        "Prevents latent collapse and mean-pose output.")
    ...
    p.add_argument("--lambda_rcxyz", type=float, default=None,
                   help="MSE on FK-derived XYZ (rot6d only). Default: 1.0 for h36m_rot6d, 0.0 otherwise.")
    p.add_argument("--lambda_velxyz", type=float, default=None,
                   help="Velocity MSE on FK XYZ (rot6d only). Default: 10.0 for h36m_rot6d, 0.0 otherwise.")
...
def main():
    ...
    if args.lr_full_enc is None:
        args.lr_full_enc = args.lr

    # ---- Mode-dependent defaults ----
    if args.data_mode == "h36m_rot6d":
        if args.lambda_rcxyz is None:
            args.lambda_rcxyz = 1.0
        if args.lambda_velxyz is None:
            args.lambda_velxyz = 10.0
        if args.lambda_vel == 5.0:
            args.lambda_vel = 1.0
    else:
        if args.lambda_rcxyz is None:
    ...
    run_tag = f"vaeac_actor_motion_{args.data_mode}_{tag_ds}_{datetime.datetime.now():%Y%m%d_%H%M%S}"
    ckpt_dir = os.path.join(args.checkpoint_dir, run_tag)
    os.makedirs(ckpt_dir, exist_ok=True)

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    # ---- Datasets (loaded first to auto-detect num_classes) -----------------
    if args.data_mode == "h36m_rot6d":
    ...
    # ---- Model --------------------------------------------------------------
    if args.data_mode == "h36m_rot6d":
        njoints, nfeats, pose_rep = 32, 6, "rot6d"
    else:
        njoints, nfeats, pose_rep = 17, 3, "xyz"

    common = dict(
        modeltype="cvae",
        njoints=njoints, nfeats=nfeats,
        num_frames=0, num_classes=args.num_classes,
        translation=True, pose_rep=pose_rep,
    ...
    module = VaeacActorMotionModule(
        ...
        lambda_kl_prior=args.lambda_kl_prior,
        ...
        lambda_velxyz=args.lambda_velxyz,
