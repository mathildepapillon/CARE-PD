...
class MotionEncoder(nn.Module):
    def __init__(self, backbone, params, num_classes=4, train_mode='end2end'):
        ...
        # When True, forward() expects pre-computed pooled features (no backbone call).
        # Set this flag after construction when using cached features.
        self.use_cached_features: bool = False
    ...
    def forward(self, x, metadata, med=None, valid_mask=None):
        ...
        x: Tensor with shape (batch_size, n_frames, n_joints, C=3) or (batch_size, n_frames, 263) for humanml3d format.
           When self.use_cached_features is True, x is expected to be a pre-pooled
           feature tensor of shape (batch_size, feature_dim) from cache_backbone_features.py,
           and the backbone + pooling steps are skipped entirely.
        """
        if self.use_cached_features:
            return self.forward_cached(x, metadata if len(self.metadata) > 0 else None)
    ...
    def forward_cached(self, cached_feat: torch.Tensor,
                       metadata: torch.Tensor | None = None) -> torch.Tensor:
        """
        Runs only the FC classifier head on pre-computed pooled backbone
        features (generated by cache_backbone_features.py).  Skips the
        backbone and the backbone-specific pooling step entirely.

        Args:
            cached_feat: (B, feature_dim) float32 tensor — pooled backbone
                         output loaded from the .npz cache.
            metadata:    (B, M) tensor or None — appended after pooling when
                         the run uses patient metadata features.
        Returns:
            (B, num_classes) logits.
        """
        feat = cached_feat
        if metadata is not None and len(self.metadata) > 0:
            feat = torch.cat([feat, metadata], dim=-1)
        return self.head._forward_fc_layers(feat)
